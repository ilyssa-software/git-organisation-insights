...
(function() {
  ...
  function setMainAppVisibility(isVisible) {
    ...
    mainAppElement.setAttribute(
      "style",
      generateMainAppElementStyle(isVisible)
    );
  }
  ...
  function generateMainAppStylesheet() {
    return (
      ...
      ".gi-done { text-decoration: line-through }" +
  ...
  function registerComponentCloseInsights() {
    Vue.component("close-insights-button", {
      ...
      template:
        '<button class="gi-close" v-on:click="hideInsights">Close Insights</button>'
    });
  }
  ...
  function registerComponentSectionGoals() {
    Vue.component("section-goals", {
      ...
      template:
        ...
        `<span v-bind:class="{ \'gi-important\': index === selectedIndex }">{{ milestone.title.substr(${
          goalMilestoneTitlePrefix.length
        }) }}</span>` +
        "</span>" +
  ...
  function registerComponentTaskFilters() {
    Vue.component("task-filters", {
      ...
      template:
        ...
        '<input type="radio" name="sprint" v-bind:value="sprintLabelTitle" v-bind:checked="index === selectedSprintIndex" v-on:click="() => populateTasks([sprintLabelTitle])" />' +
  ...
  function populateGoals() {
    fetchMilestones(groupName, milestones => {
      mainApp.milestones = milestones.filter(milestone =>
        milestone.title.startsWith(goalMilestoneTitlePrefix)
      );
    });
  }
  ...
  function populateTasks(labelTitles) {
    ...
    for (const repoName of repoNames) {
      fetchIssues(repoName, labelTitles, issues =>
        handleFetchedIssues(repoName, issues)
      );
    }
  }
  ...
  function fetchIssues(repoName, labelTitles, completion) {
    ...
    const labelTitlesQueryPart = labelTitles
      .map(labelTitle => `label_name[]=${encodeURI(labelTitle)}`)
      .join("&");
    GM_xmlhttpRequest({
      method: "GET",
  ...
  function parseMilestonesResponseText(responseText) {
    ...
    const milestoneElements = Array.from(
      rootElement.getElementsByClassName("milestone-open")
    );

    return milestoneElements.map(milestoneElement =>
      parseMilestoneElement(milestoneElement)
    );
  }

  function parseMilestoneElement(milestoneElement) {
    const milestoneRangeElement = milestoneElement.getElementsByClassName(
      "milestone-range"
    )[0];
    return {
      title: milestoneElement.getElementsByClassName("append-bottom-5")[0]
        .children[0].children[0].textContent,
      url: milestoneElement
        .getElementsByClassName("append-bottom-5")[0]
  ...
  function parseIssuesResponseText(responseText) {
    ...
    const issueElements = Array.from(
      rootElement.getElementsByClassName("issuable-info-container")
    );
  ...
  function parseIssueElement(issueElement) {
    const milestoneElement = issueElement.getElementsByClassName(
      "issuable-milestone"
    )[0];
    const issueTitleTextElement = issueElement.getElementsByClassName(
      "issue-title-text"
    )[0];
    return {
      reference: issueElement
        .getElementsByClassName("issuable-reference")[0]
        .textContent.trim(),
      url: issueTitleTextElement
        .getElementsByTagName("a")[0]
        .getAttribute("href"),
      title: issueTitleTextElement.children[
        issueTitleTextElement.children.length - 1
      ].textContent.trim(),
      author: issueElement
        .getElementsByClassName("author")[0]
        .textContent.trim(),
      assignee: parseAuthorLinkElement(
        issueElement.getElementsByClassName("author-link")[1]
      ),
      labels: Array.from(issueElement.getElementsByClassName("label-link")).map(
        element => parseLabelLinkElement(element)
      ),
      milestoneTitle:
        milestoneElement && milestoneElement.children[0].textContent.trim(),
  ...
  function parseLabelLinkElement(labelLinkElement) {
    return {
      ...
      color: /.*background-color:\s*(.*);.*/g.exec(
        labelLinkElement.children[0].getAttribute("style")
      )[1],
